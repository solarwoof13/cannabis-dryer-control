...
elif 'arm' in platform.machine() or 'aarch' in platform.machine():  # Raspberry Pi
    DB_PATH = Path('/home/pi/cannabis-dryer-control/data/settings.db')
else:  # Other Linux/Windows
    DB_PATH = Path('./data/settings.db')
...
def emergency_stop():
    """Emergency stop - halt all equipment"""
    try:
        # This would interface with your equipment control
        # For now, just log and update status
        update_setting('equipment', 'manual_override', True)
        update_setting('process', 'session_status', 'emergency_stopped')
